...
# ✅ UPDATE 1: Accept Session ID in Header
@app.post("/upload")
async def upload_pdf(
    file: UploadFile = File(...), 
    x_session_id: str = Header(default="default") # <--- NEW
):
    print(f"📥 Received file: {file.filename} for Session: {x_session_id}")
    ...
    for i, chunk in enumerate(chunks): 
        ...
        if vector:
            vectors.append({
                "id": f"{x_session_id}_{file.filename}_{i}", # Make ID unique per user
                "values": vector,
                "metadata": {"text": chunk}
            })
            time.sleep(0.2) 
    ...
    if vectors:
        try:
            ...
            for i in range(0, len(vectors), batch_size):
                ...
                # ✅ KEY FIX: Save to User's Namespace
...
# ✅ UPDATE 2: Accept Session ID in Header
@app.post("/chat")
async def chat(
    query: Query, 
    x_session_id: str = Header(default="default") # <--- NEW
):
    print(f"💬 Question: {query.question} from Session: {x_session_id}")
    
    q_embedding = get_embedding(query.question)
    if not q_embedding:
        return {"answer": "⚠️ Error: Embedding model failed."}

    # ✅ KEY FIX: Search ONLY in User's Namespace
